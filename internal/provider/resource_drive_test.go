...
func TestAccDrive_CreateWithOrgUnitId(t *testing.T) {
	var updateBody []byte
	...
	server := setupTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == "PATCH" && strings.Contains(r.URL.Path, "/drives/drive-ou"):
			body, _ := io.ReadAll(r.Body)
			updateBody = body
			jsonResponse(w, 200, map[string]any{
				"kind":      "drive#drive",
				"id":        "drive-ou",
				"name":      "OrgUnit Drive",
				"orgUnitId": "ou-engineering-123",
			})

		case r.Method == "GET" && strings.Contains(r.URL.Path, "/drives/drive-ou"):
			jsonResponse(w, 200, map[string]any{
				"kind":      "drive#drive",
				"id":        "drive-ou",
				"name":      "OrgUnit Drive",
		...
		case r.Method == "DELETE" && strings.Contains(r.URL.Path, "/drives/drive-ou"):
			w.WriteHeader(204)
...
resource "googleworkspace_drive" "test" {
  name        = "OrgUnit Drive"
  org_unit_id = "ou-engineering-123"
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("googleworkspace_drive.test", "id", "drive-ou"),
					resource.TestCheckResourceAttr("googleworkspace_drive.test", "org_unit_id", "ou-engineering-123"),
				),
			},
		},
	})
	...
	if reqBody["orgUnitId"] != "ou-engineering-123" {
		t.Errorf("expected orgUnitId in update body, got %v", reqBody["orgUnitId"])
	}
}

func TestAccDrive_UpdateOrgUnitId(t *testing.T) {
	var updateCount atomic.Int32
	var lastUpdateBody []byte
	...
	server := setupTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == "PATCH" && strings.Contains(r.URL.Path, "/drives/drive-ou-upd"):
			body, _ := io.ReadAll(r.Body)
			lastUpdateBody = body
			updateCount.Add(1)
			...
			if ou, ok := req["orgUnitId"]; ok {
				resp["orgUnitId"] = ou
			}
			jsonResponse(w, 200, resp)

		case r.Method == "GET" && strings.Contains(r.URL.Path, "/drives/drive-ou-upd"):
			resp := map[string]any{
				"kind": "drive#drive",
				"id":   "drive-ou-upd",
				"name": "OU Update",
			...
			if count >= 2 {
				resp["orgUnitId"] = "ou-sales-456"
			} else if count >= 1 {
				resp["orgUnitId"] = "ou-engineering-123"
			}
			jsonResponse(w, 200, resp)

		case r.Method == "DELETE" && strings.Contains(r.URL.Path, "/drives/drive-ou-upd"):
			w.WriteHeader(204)
...
`,
				Check: resource.TestCheckResourceAttr("googleworkspace_drive.test", "org_unit_id", "ou-engineering-123"),
			},
			{
				Config: testProviderConfig + `
resource "googleworkspace_drive" "test" {
  name        = "OU Update"
  org_unit_id = "ou-sales-456"
}
`,
				Check: resource.TestCheckResourceAttr("googleworkspace_drive.test", "org_unit_id", "ou-sales-456"),
			},
		},
	})
	...
	if reqBody["orgUnitId"] != "ou-sales-456" {
		t.Errorf("expected orgUnitId=ou-sales-456 in final update, got %v", reqBody["orgUnitId"])
	}
}
